        ...
        <CommandPrimitive className="flex h-full w-full flex-col overflow-hidden rounded-[26px]">
          <div className="p-8 pb-4">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h2 className="text-3xl font-serif text-[#120D0A]">
                  Search PASHAN
                </h2>
                <p className="text-sm text-[#756E64] mt-1">
                  Find your bracelet, gemstone or intention.
                </p>
          ...
          <CommandPrimitive.List className="px-6 pb-6 max-h-[60vh] overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-[#C3723C] [&::-webkit-scrollbar-track]:bg-transparent">
            <CommandPrimitive.Empty className="py-12 text-center">
              <p className="text-lg font-serif text-[#120D0A]">
                No matching stone found.
              </p>
              <p className="text-sm text-[#756E64] mt-2">
                Try searching by stone name, emotion, or intention.
              </p>
              <div className="flex flex-wrap gap-2 mt-6 justify-center">
                {[
                  ...
                  <button
                    key={chip}
                    className="px-4 py-2 rounded-full border border-[#C3723C]/30 text-sm text-[#B76E45] hover:bg-[#C3723C]/10 transition-colors"
                  >
                    {chip}
                  </button>
                ))}
            ...
            <CommandPrimitive.Group
              heading="Popular"
              className="text-xs uppercase tracking-widest text-[#B76E45] font-medium mb-4 mt-4"
            >
              <div className="flex flex-wrap gap-2 mt-2">
                {popularSearches.map((item) => (
                  <CommandPrimitive.Item
                    key={item}
                    className="px-4 py-2 rounded-full border border-[#C3723C]/30 text-sm text-[#B76E45] cursor-pointer hover:bg-[#C3723C]/10 transition-colors"
                  >
                    {item}
                  </CommandPrimitive.Item>
                ))}
