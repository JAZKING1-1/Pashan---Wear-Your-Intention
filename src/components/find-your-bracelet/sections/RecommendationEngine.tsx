...
export function RecommendationEngine({ selectedIntentions, bracelets }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 md:p-12 bg-[#D1C9B8] text-[#2E1A14] rounded-2xl shadow-xl">
      <h3 className="font-serif text-4xl mb-10 text-center uppercase tracking-widest">
        Consultation Result
      </h3>
      ...
      <div className="grid md:grid-cols-2 gap-12 items-center mb-16">
        <img
          src={bestMatch.bracelet.image}
          alt={bestMatch.bracelet.stone}
          className="rounded-xl shadow-2xl"
        />
        <div>
          <h4 className="text-sm uppercase tracking-widest text-[#B87333] mb-2 font-bold">
            Best Match for you
          </h4>
          <h4 className="text-4xl font-serif mb-4">
            {bestMatch.bracelet.stone}
          </h4>
          <div className="w-full bg-[#2E1A14]/20 h-2 rounded-full mb-6">
            <div
              className="bg-[#B87333] h-2 rounded-full"
              style={{ width: `${bestMatch.percentage}%` }}
            ></div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {alternatives.map((alt) => (
              <div key={alt.bracelet.slug} className="flex gap-6 items-center">
                <img
                  src={alt.bracelet.image}
                  alt={alt.bracelet.stone}
                ...
                <div>
                  ...
                  <p className="text-sm text-[#2E1A14]/60 mb-3">
