...
export function Navbar() {
  ...
  const navLinkClass = "text-ivory text-sm uppercase tracking-[0.1em] hover:text-gold-soft transition-colors relative after:content-[''] after:absolute after:bottom-[-4px] after:left-1/2 after:w-0 after:h-[1px] after:bg-gold after:transition-all after:duration-220 hover:after:left-0 hover:after:w-full";

  return (
    <header className={`sticky top-0 w-full z-[1000] transition-all duration-300 bg-[#2B160F] ${scrolled ? "backdrop-blur-md bg-[#2B160F]/95" : ""}`}>
      <div className="container-luxe mx-auto flex items-center justify-between h-[90px] border-b border-[#C8A15A]/30">
        ...
        <div className="flex items-center gap-6 text-ivory">
            ...
            <button onClick={() => setOpen(true)} className="relative hover:scale-105 transition-transform" aria-label="Cart">
                <ShoppingBag size={20} />
                {count > 0 && <span className="absolute -top-2 -right-2 bg-gold text-bg-dark text-xs w-4 h-4 rounded-full flex items-center justify-center">{count}</span>}
            </button>
            <button className="md:hidden" onClick={() => setMobileOpen(true)} aria-label="Open Menu"><Menu size={24} /></button>
        </div>
      </div>
